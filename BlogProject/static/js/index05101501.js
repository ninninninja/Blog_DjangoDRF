function ShowArticles(data) {
        ...
        $.each(arr, function(i) {
            ...
            '<a type="button" class="article-link" id="'+__ID__+'"><h2 class="post-title" id="post-title-'+i+'"></h2></a>',
...
$(document).ready(function(){
    ...
    $.get(url, function(data) {
        ...
        $('.article-link').click( function () {
            ...
            $.ajax({
                url: '/api/article/' + select_article + '/',
                method: 'GET'
            }).success(function (data, textStatus, jqXHR) {
                alert($('#topic').text());
                window.location = '/tmp-url/14';
                alert('333');
                alert($('#topic').text());
